<nav class="bg-indigo-800 border-gray-200 dark:bg-gray-900">
            ...
            <ul
                ...
                @auth
                    <x-navbar-link href="/" :active="request()->is('/')"
                        class="text-white hover:text-indigo-300 transition-colors">Home</x-navbar-link>
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit" class="text-white transition-colors hover:text-indigo-300">Logout</button>
                    </form>
                @endauth
            </ul>
        </div>
